...
class DbusAdapter extends models_1.Adapter {
    constructor(noble, id, name, address, busObject) {
        super(noble, id);
    ...
    prop(propName) {
        return this.busObject.prop(misc_1.I_BLUEZ_ADAPTER, propName);
    }
    callMethod(methodName, ...args) {
        return this.busObject.callMethod(misc_1.I_BLUEZ_ADAPTER, methodName, ...args);
    }
    ...
    async updatePeripherals() {
        ...
        for (const peripheralId of peripheralIds) {
            ...
            if (!peripheral) {
                ...
                const address = await busObject.prop(misc_1.I_BLUEZ_DEVICE, 'Address');
                const addressType = await busObject.prop(misc_1.I_BLUEZ_DEVICE, 'AddressType');
                peripheral = new Peripheral_1.DbusPeripheral(this, peripheralId, address, addressType, busObject);
