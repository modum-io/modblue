...
class Hci extends events_1.EventEmitter {
	...
	async sendCommand(name, data, onlyStatus) {
			...
			this.cmds.set(cmd, {
				...
				onStatus: (status) => {
					if (onlyStatus) {
						status !== 0
							? reject(
									new Error(`Command ${cmd} status ${hci_status_json_1.default[status]} (0x${status.toString(16)})`)
							  )
							: resolve();
						this.cmds.delete(cmd);
					}
				...
				onResponse: (status, responseData) =>
					status !== 0
						? reject(
								new Error(`Command ${cmd} response ${hci_status_json_1.default[status]} (0x${status.toString(16)})`)
						  )
