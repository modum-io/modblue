...
class Hci extends events_1.EventEmitter {
    ...
    async sendCommand(data, onStatus, onResponse) {
    ...
    async createLeConn(address, addressType) {
        ...
        const origScope = new Error();
        return this.sendCommand(cmd, (cmdStatus, resolve, reject) => {
            if (cmdStatus !== 0) {
                const errStatus = `${hci_status_json_1.default[cmdStatus]} (0x${cmdStatus.toString(16).padStart(2, '0')})`;
    ...
    async disconnect(handle, reason) {
        ...
        await this.sendCommand(cmd, (cmdStatus, resolve, reject) => {
            ...
            const onComplete = (status, _handle, _reason) => {
                if (_handle !== handle) {
                    return;
                }
