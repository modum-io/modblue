...
class Hci extends events_1.EventEmitter {
    constructor(deviceId) {
        ...
        this.cmdMutex = async_mutex_1.withTimeout(new async_mutex_1.Mutex(), 30000, new Error(`HCI command mutex timeout`));
        this.pendingCmd = null;
    }
    ...
    async sendCommand(data, onlyStatus) {
        ...
        return new Promise((resolve, reject) => {
            const onDone = (status, responseData) => {
                this.pendingCmd = null;
                release();
                if (status !== 0) {
                    reject(new Error(`HCI Command failed: ${hci_status_json_1.default[status]} (0x${status.toString(16).padStart(2, '0')})`));
                }
                else {
    ...
    async waitForLeMetaEvent(metaEvent) {
        return new Promise((resolve, reject) => {
            const handler = (status, data) => {
                ...
                if (status !== 0) {
                    reject(new Error(`Received LE error ${hci_status_json_1.default[status]} (0x${status.toString(16).padStart(2, '0')})`));
                }
                else {
                    resolve(data);
                }
    ...
    async createLeConn(address, addressType) {
        ...
        return new Promise((resolve, reject) => {
            const onComplete = (status, handle, role, _addressType, _address) => {
                ...
                if (status !== 0) {
                    reject(new Error(`LE conn failed: ${hci_status_json_1.default[status]} (0x${status.toString(16).padStart(2, '0')})`));
                    return;
                }
                if (role !== 0) {
    ...
    processLeConnComplete(status, data) {
        ...
        this.emit('leConnComplete', status, handleId, role, addressType, address, interval, latency, supervisionTimeout, masterClockAccuracy);
    }
