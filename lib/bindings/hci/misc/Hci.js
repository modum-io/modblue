...
class Hci extends tiny_typed_emitter_1.TypedEmitter {
    constructor(devId, cmdTimeout = HCI_CMD_TIMEOUT, connTimeout = HCI_CONN_TIMEOUT) {
        ...
        this.cmdMutex = async_mutex_1.withTimeout(new async_mutex_1.Mutex(), this.cmdTimeout, new HciError_1.HciError(`HCI command mutex timeout`));
        this.connTimeout = connTimeout;
        this.connMutex = async_mutex_1.withTimeout(new async_mutex_1.Mutex(), this.connTimeout, new HciError_1.HciError(`HCI connection mutex timeout`));
    }
    static createSocket() {
        let Socket;
        try {
