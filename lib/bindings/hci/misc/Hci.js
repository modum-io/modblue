...
class Hci extends tiny_typed_emitter_1.TypedEmitter {
    constructor(devId, cmdTimeout = HCI_CMD_TIMEOUT, connTimeout = HCI_CONN_TIMEOUT) {
        super();
        this.aclPacketQueue = [];
        ...
        this.cmdMutex = (0, async_mutex_1.withTimeout)(new async_mutex_1.Mutex(), this.cmdTimeout, new HciError_1.HciError(`HCI command mutex timeout`));
        this.connTimeout = connTimeout;
        this.connMutex = (0, async_mutex_1.withTimeout)(new async_mutex_1.Mutex(), this.connTimeout, new HciError_1.HciError(`HCI connection mutex timeout`));
    }
    static createSocket() {
        let Socket;
        try {
        ...
        if (!Socket) {
            throw new Error(`Could not find any socket implementations. Please install @modum-io/bluetooth-hci-socket or @abandonware/bluetooth-hci-socket`);
        }
        return new Socket();
    }
    ...
    sendCommand(data, statusOnly) {
        return __awaiter(this, void 0, void 0, function* () {
            // Capture original scope of function call
            const origScope = new Error();
            // Check if our socket is available
            if (!this.isSocketUp) {
                throw new HciError_1.HciError('HCI socket is not available');
            }
            const release = yield this.cmdMutex.acquire();
    ...
    setScanEnabled(enabled, filterDuplicates) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const release = yield this.connMutex.acquire();
            try {
                yield this.sendCommand(cmd);
            }
            finally {
    ...
    cancelLeConn() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // length
            cmd.writeUInt8(0x00, 3);
            yield this.sendCommand(cmd, false);
        });
    }
