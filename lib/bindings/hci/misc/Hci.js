...
class Hci extends events_1.EventEmitter {
    constructor(deviceId, cmdTimeout) {
        super();
        this.aclPacketQueue = [];
        ...
        this.mutex = async_mutex_1.withTimeout(new async_mutex_1.Mutex(), this.cmdTimeout, new Error(`HCI command mutex timeout`));
        this.currentCmd = null;
    }
    ...
    async init() {
        ...
        if (!this.isSocketUp) {
            throw new Error(`Initializing socket timed out - Are you sure it's running? On unix try \`sudo hciconfig hci${this.deviceId} up\``);
        }
    }
    dispose() {
        if (this.socketTimer) {
    ...
    async sendCommand(data, statusOnly, customMutex) {
        ...
        if (!customMutex) {
            if (!release) {
                throw new HciError_1.HciError(`Could not aquire HCI command mutex`, (_a = this.mutexStack) === null || _a === void 0 ? void 0 : _a.stack);
            }
