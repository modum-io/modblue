...
class Gatt extends events_1.EventEmitter {
    ...
    async queueCommandAsync(buffer, resolveOnWrite) {
    ...
    async doDiscoverServices() {
        ...
        while (true) {
            const data = await this.queueCommandAsync(this.readByGroupRequest(startHandle, 0xffff, GATT_PRIM_SVC_UUID), false);
            const opcode = data[0];
    ...
    async discoverCharacteristics(serviceUUID, characteristicUUIDs) {
        const service = this.services.get(serviceUUID);
        const characteristics = await this.doDiscoverCharacteristics(serviceUUID);
    ...
    async doDiscoverCharacteristics(serviceUUID) {
        const service = this.services.get(serviceUUID);
        ...
        while (true) {
            const data = await this.queueCommandAsync(this.readByTypeRequest(startHandle, service.endHandle, GATT_CHARAC_UUID), false);
            const opcode = data[0];
            if (opcode === ATT_OP_READ_BY_TYPE_RESP) {
                const type = data[1];
                const num = (data.length - 2) / type;
