...
class HciGattLocal extends models_1.Gatt {
    ...
    setData(deviceName, services) {
        ...
        for (const service of allServices) {
            const newChars = [];
            const newService = new models_1.GattService(this, service.uuid, false, newChars);
            ...
            for (const char of service.characteristics) {
                ...
                const newChar = new models_1.GattCharacteristic(newService, char.uuid, false, char.properties, char.secure, onRead, onWrite, newDescriptors);
                ...
                if (char.properties.includes('indicate') || char.properties.includes('notify')) {
                    ...
                    const newDescr = new models_1.GattDescriptor(newChar, '2902', false, Buffer.from([0x00, 0x00]));
                ...
                if (char.descriptors) {
                    for (const descr of char.descriptors) {
                        const newDescr = new models_1.GattDescriptor(newChar, descr.uuid, false, descr.value);
