...
class HciGattRemote extends models_1.Gatt {
    constructor(peripheral, hci, handle, cmdTimeout = GATT_CMD_TIMEOUT) {
        ...
        this.onAclStreamData = (handle, cid, data) => {
            if (handle !== this.handle || cid !== misc_1.Codes.ATT_CID) {
                return;
            }
            if (this.currentCmd && data.toString('hex') === this.currentCmd.buffer.toString('hex')) {
                ...
                this.queueCommand(this.errorResponse(requestType, 0x0000, misc_1.Codes.ATT_ECODE_REQ_NOT_SUPP), true).catch(() => null);
            }
            else if (data[0] === misc_1.Codes.ATT_OP_HANDLE_NOTIFY || data[0] === misc_1.Codes.ATT_OP_HANDLE_IND) {
                const valueHandle = data.readUInt16LE(1);
                const valueData = data.slice(3);
                // this.emit('handleNotify', valueHandle, valueData);
                if (data[0] === misc_1.Codes.ATT_OP_HANDLE_IND) {
                ...
                if (data[0] === misc_1.Codes.ATT_OP_ERROR &&
                    (data[4] === misc_1.Codes.ATT_ECODE_AUTHENTICATION ||
                        data[4] === misc_1.Codes.ATT_ECODE_AUTHORIZATION ||
                        data[4] === misc_1.Codes.ATT_ECODE_INSUFF_ENC) &&
        ...
        this.mutex = async_mutex_1.withTimeout(new async_mutex_1.Mutex(), this.cmdTimeout, new models_1.GattError(peripheral, 'GATT command mutex timeout'));
        this.currentCmd = null;
    ...
    acquireMutex() {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                throw new models_1.GattError(this.peripheral, `Could not acquire GATT command mutex`, (_a = this.mutexStack) === null || _a === void 0 ? void 0 : _a.stack);
            }
        });
    }
    dispose(reason) {
    ...
    queueCommand(buffer, resolveOnWrite) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const gattError = new models_1.GattError(this.peripheral, 'GATT Error'); // Actual error will be appended
            const timeoutError = new models_1.GattError(this.peripheral, 'GATT command timed out');
            return new Promise((resolve, reject) => {
                ...
                this.hci.writeAclDataPkt(this.handle, misc_1.Codes.ATT_CID, buffer);
    ...
    readByTypeRequest(startHandle, endHandle, groupUUID) {
        const buf = Buffer.alloc(7);
        buf.writeUInt8(misc_1.Codes.ATT_OP_READ_BY_TYPE_REQ, 0);
        buf.writeUInt16LE(startHandle, 1);
        buf.writeUInt16LE(endHandle, 3);
        buf.writeUInt16LE(groupUUID, 5);
        return this.queueCommand(buf, false);
    ...
    readRequest(handle) {
        const buf = Buffer.alloc(3);
        buf.writeUInt8(misc_1.Codes.ATT_OP_READ_REQ, 0);
        buf.writeUInt16LE(handle, 1);
        return this.queueCommand(buf, false);
    }
    readBlobRequest(handle, offset) {
        const buf = Buffer.alloc(5);
        buf.writeUInt8(misc_1.Codes.ATT_OP_READ_BLOB_REQ, 0);
        buf.writeUInt16LE(handle, 1);
    ...
    findInfoRequest(startHandle, endHandle) {
        const buf = Buffer.alloc(5);
        buf.writeUInt8(misc_1.Codes.ATT_OP_FIND_INFO_REQ, 0);
        buf.writeUInt16LE(startHandle, 1);
        buf.writeUInt16LE(endHandle, 3);
        return this.queueCommand(buf, false);
    }
    writeRequest(handle, data, withoutResponse) {
        ...
        buf.writeUInt8(withoutResponse ? misc_1.Codes.ATT_OP_WRITE_CMD : misc_1.Codes.ATT_OP_WRITE_REQ, 0);
        buf.writeUInt16LE(handle, 1);
    ...
    prepareWriteRequest(handle, offset, data) {
        ...
        buf.writeUInt8(misc_1.Codes.ATT_OP_PREPARE_WRITE_REQ, 0);
        buf.writeUInt16LE(handle, 1);
        buf.writeUInt16LE(offset, 3);
        for (let i = 0; i < data.length; i++) {
    ...
    executeWriteRequest(handle, cancelPreparedWrites) {
        ...
        buf.writeUInt8(misc_1.Codes.ATT_OP_EXECUTE_WRITE_REQ, 0);
        buf.writeUInt8(cancelPreparedWrites ? 0 : 1, 1);
        return this.queueCommand(buf, false);
    }
    handleConfirmation() {
        const buf = Buffer.alloc(1);
        buf.writeUInt8(misc_1.Codes.ATT_OP_HANDLE_CNF, 0);
        return this.queueCommand(buf, true);
    }
    exchangeMtu(mtu) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const opcode = data[0];
            if (opcode === misc_1.Codes.ATT_OP_MTU_RESP) {
                const newMtu = data.readUInt16LE(1);
                this._mtu = Math.min(mtu, newMtu);
                this.mtuWasExchanged = true;
            }
            else {
                throw new models_1.GattError(this.peripheral, 'Exchanging mtu failed');
            }
            return this.mtu;
        });
    }
    discoverServices() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const run = true;
            while (run) {
                const data = yield this.readByGroupRequest(startHandle, 0xffff, misc_1.Codes.GATT_PRIM_SVC_UUID);
                const opcode = data[0];
                if (opcode === misc_1.Codes.ATT_OP_READ_BY_GROUP_RESP) {
                    const type = data[1];
                    ...
                    for (let i = 0; i < num; i++) {
                        ...
                        const uuid = type === 6
                            ...
                            : data
                                ...
                                .reverse()
                                .join('');
                        const newService = new Service_1.HciGattService(this, uuid, true, srvStartHandle, srvEndHandle);
                        newServices.push(newService);
                    }
                }
                if (opcode !== misc_1.Codes.ATT_OP_READ_BY_GROUP_RESP || newServices[newServices.length - 1].endHandle === 0xffff) {
                    break;
                }
                else {
    ...
    discoverCharacteristics(serviceUUID) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!service) {
                throw new models_1.GattError(this.peripheral, `Service ${serviceUUID} not found`);
            }
            const newChars = [];
            let startHandle = service.startHandle;
            const run = true;
            while (run) {
                const data = yield this.readByTypeRequest(startHandle, service.endHandle, misc_1.Codes.GATT_CHARAC_UUID);
                const opcode = data[0];
                if (opcode === misc_1.Codes.ATT_OP_READ_BY_TYPE_RESP) {
                    const type = data[1];
                    const num = (data.length - 2) / type;
                    for (let i = 0; i < num; i++) {
                        const offset = 2 + i * type;
                        ...
                        const uuid = type === 7
                            ...
                            : data
                                ...
                                .reverse()
                                .join('');
                        const newChar = new Characteristic_1.HciGattCharacteristic(service, uuid, true, propertiesFlag, secureFlag, charStartHandle, charValueHandle);
    ...
    readCharacteristic(serviceUUID, characteristicUUID) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!characteristic) {
                throw new models_1.GattError(this.peripheral, `Characteristic ${characteristicUUID} in service ${serviceUUID} not found`);
            }
            ...
            const run = true;
            while (run) {
                if (opcode !== misc_1.Codes.ATT_OP_READ_RESP && opcode !== misc_1.Codes.ATT_OP_READ_BLOB_RESP) {
                    return readData;
                }
    ...
    writeCharacteristic(serviceUUID, characteristicUUID, data, withoutResponse) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (withoutResponse) {
                ...
                if (opcode !== misc_1.Codes.ATT_OP_WRITE_RESP) {
                    throw new models_1.GattError(this.peripheral, `Write error, opcode ${opcode}`);
                }
            }
        });
    }
    ...
    longWrite(serviceUUID, characteristicUUID, data, withoutResponse) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            while (offset < data.length) {
                ...
                if (chunkOpcode !== misc_1.Codes.ATT_OP_PREPARE_WRITE_RESP) {
                    throw new models_1.GattError(this.peripheral, `Long write chunk failed, invalid opcode ${chunkOpcode}`);
                }
                else {
                    ...
                    if (chunkRespData.length !== expectedLength) {
                        ...
                        throw new models_1.GattError(this.peripheral, `Long write chunk failed, received invalid response length`);
                    }
                }
                offset = end;
            }
            ...
            const opcode = respData[0];
            if (opcode !== misc_1.Codes.ATT_OP_EXECUTE_WRITE_RESP && !withoutResponse) {
                throw new models_1.GattError(this.peripheral, `Long write failed, invalid opcode ${opcode}`);
            }
        });
    }
    broadcastCharacteristic(serviceUUID, characteristicUUID, broadcast) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const data = yield this.readByTypeRequest(characteristic.startHandle, characteristic.endHandle, misc_1.Codes.GATT_SERVER_CHARAC_CFG_UUID);
            const opcode = data[0];
            if (opcode !== misc_1.Codes.ATT_OP_READ_BY_TYPE_RESP) {
                throw new models_1.GattError(this.peripheral, `Broadcast error, opcode ${opcode}`);
            }
            ...
            if (moreOpcode !== misc_1.Codes.ATT_OP_WRITE_RESP) {
                throw new models_1.GattError(this.peripheral, `Broadcast error, opcode ${opcode}`);
            }
        });
    }
    notifyCharacteristic(serviceUUID, characteristicUUID, notify) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const data = yield this.readByTypeRequest(characteristic.startHandle, characteristic.endHandle, misc_1.Codes.GATT_CLIENT_CHARAC_CFG_UUID);
            const opcode = data[0];
            if (opcode === misc_1.Codes.ATT_OP_READ_BY_TYPE_RESP) {
                const handle = data.readUInt16LE(2);
                let value = data.readUInt16LE(4);
                ...
                const moreOpcode = moreData[0];
                if (moreOpcode !== misc_1.Codes.ATT_OP_WRITE_RESP) {
                    throw new models_1.GattError(this.peripheral, `Notify error, opcode ${opcode}`);
                }
            }
        });
    }
    discoverDescriptors(serviceUUID, characteristicUUID) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            while (run) {
                ...
                const opcode = data[0];
                if (opcode === misc_1.Codes.ATT_OP_FIND_INFO_RESP) {
                    const num = data[1];
                    for (let i = 0; i < num; i++) {
                        const descHandle = data.readUInt16LE(2 + i * 4 + 0);
                        const uuid = data.readUInt16LE(2 + i * 4 + 2).toString(16);
                        const newDescriptor = new Descriptor_1.HciGattDescriptor(characteristic, uuid, true, descHandle);
                        newDescs.push(newDescriptor);
                    }
                }
                if (opcode !== misc_1.Codes.ATT_OP_FIND_INFO_RESP || newDescs[newDescs.length - 1].handle === characteristic.endHandle) {
                    break;
                }
                else {
    ...
    readDescriptor(serviceUUID, characteristicUUID, descriptorUUID) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!descriptor) {
                throw new models_1.GattError(this.peripheral, `Descriptor ${descriptorUUID} in characteristic ${characteristicUUID} in service ${serviceUUID} not found`);
            }
            let readData = Buffer.alloc(0);
    ...
    writeDescriptor(serviceUUID, characteristicUUID, descriptorUUID, data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const opcode = respData[0];
            if (opcode !== misc_1.Codes.ATT_OP_WRITE_RESP) {
                throw new models_1.GattError(this.peripheral, `WriteValue error, opcode ${opcode}`);
            }
        });
    }
    addService() {
