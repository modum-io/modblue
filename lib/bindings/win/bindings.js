...
const rt_utils_1 = __importDefault(require("./rt-utils"));
...
rt_utils_1.default.using(importPrefix + 'Windows.Foundation');
rt_utils_1.default.using(importPrefix + 'Windows.Storage.Streams');
rt_utils_1.default.using(importPrefix + 'Windows.Devices.Enumeration');
rt_utils_1.default.using(importPrefix + 'Windows.Devices.Bluetooth.GenericAttributeProfile');
rt_utils_1.default.using(importPrefix + 'Windows.Devices.Bluetooth');
rt_utils_1.default.using(importPrefix + 'Windows.Devices.Bluetooth.Advertisement');
rt_utils_1.default.using(importPrefix + 'Windows.Devices.Radios');
...
util_1.default.inherits(NobleBindings, events_1.default.EventEmitter);
NobleBindings.prototype.init = function () {
    ...
    debug('initialized');
    rt_utils_1.default.promisify(Radio.getRadiosAsync)()
        .then((radiosList) => {
        radiosList = rt_utils_1.default.toArray(radiosList);
...
NobleBindings.prototype.startScanning = function (serviceUuids, allowDuplicates) {
    ...
    rt_utils_1.default.keepAlive(true);
};
NobleBindings.prototype.stopScanning = function () {
    if (this._advertisementWatcher.status === BluetoothLEAdvertisementWatcherStatus.started) {
        debug('stopScanning()');
        this._advertisementWatcher.stop();
        rt_utils_1.default.keepAlive(false);
    }
};
NobleBindings.prototype.connect = function (deviceUuid) {
    ...
    rt_utils_1.default.promisify(BluetoothLEDevice.fromBluetoothAddressAsync)(deviceRecord.address)
        .then((device) => {
        if (device) {
            debug('got bluetooth device: %s (%s)', device.name, device.deviceInformation.kind);
            /*const pairing = device.deviceInformation.pairing;
            debug('pairing %s %s %s', pairing.isPaired, pairing.canPair, pairing.protectionLevel);
            if (!pairing.isPaired) {
                const e = new Error('Device is not paired: ' + deviceRecord.formattedAddress);
                e.deviceNotPaired = true;
                throw e;
            }*/
            deviceRecord.device = rt_utils_1.default.trackDisposable(deviceUuid, device);
...
NobleBindings.prototype.disconnect = function (deviceUuid) {
    ...
    if (deviceRecord.device) {
        ...
        rt_utils_1.default.disposeAll(deviceUuid);
        rt_utils_1.default.keepAlive(false);
...
NobleBindings.prototype.discoverServices = function (deviceUuid, filterServiceUuids) {
    ...
    if (filterServiceUuids && filterServiceUuids.length === 1) {
        rt_utils_1.default.promisify(device.getGattServicesForUuidAsync, device)(addDashesToUuid(filterServiceUuids[0]))
            .then((result) => {
            checkCommunicationResult(deviceUuid, result);
            let services = rt_utils_1.default.trackDisposables(deviceUuid, rt_utils_1.default.toArray(result.services));
    ...
    rt_utils_1.default.promisify(device.getGattServicesAsync, device)(BluetoothCacheMode.uncached)
        .then((result) => {
        checkCommunicationResult(deviceUuid, result);
        let services = rt_utils_1.default.trackDisposables(deviceUuid, rt_utils_1.default.toArray(result.services));
        let serviceUuids = services.map((s) => uuidToString(s.uuid)).filter(filterUuids(filterServiceUuids));
        debug(deviceUuid + ' services: %o', serviceUuids);
        this.emit('servicesDiscover', deviceUuid, serviceUuids);
    })
        .catch((ex) => {
...
NobleBindings.prototype.discoverIncludedServices = function (deviceUuid, serviceUuid, filterServiceUuids) {
    ...
    this._getCachedServiceAsync(deviceUuid, serviceUuid)
        .then((service) => {
        rt_utils_1.default.promisify(service.getIncludedServicesAsync, service)(BluetoothCacheMode.uncached).then((result) => {
            checkCommunicationResult(deviceUuid, result);
            let includedServices = rt_utils_1.default.trackDisposables(deviceUuid, rt_utils_1.default.toArray(result.services));
            let includedServiceUuids = includedServices
...
NobleBindings.prototype.discoverCharacteristics = function (deviceUuid, serviceUuid, filterCharacteristicUuids) {
    ...
    this._getCachedServiceAsync(deviceUuid, serviceUuid)
        ...
        return rt_utils_1.default
            ...
            debug('%s %s characteristics: %o', deviceUuid, serviceUuid, characteristics.map((c) => c.uuid));
...
NobleBindings.prototype.read = function (deviceUuid, serviceUuid, characteristicUuid) {
    ...
    this._getCachedCharacteristicAsync(deviceUuid, serviceUuid, characteristicUuid)
        ...
        return rt_utils_1.default
            ...
            .then((result) => {
            checkCommunicationResult(deviceUuid, result);
            let data = rt_utils_1.default.toBuffer(result.value);
            debug('  => [' + data.length + ']');
...
NobleBindings.prototype.write = function (deviceUuid, serviceUuid, characteristicUuid, data, withoutResponse) {
    ...
    this._getCachedCharacteristicAsync(deviceUuid, serviceUuid, characteristicUuid)
        .then((characteristic) => {
        let rtBuffer = rt_utils_1.default.fromBuffer(data);
        return rt_utils_1.default
        ...
        if (notify) {
            ...
            return rt_utils_1.default
                ...
                listener = ((source, e) => {
                    ...
                    let data = rt_utils_1.default.toBuffer(e.characteristicValue);
...
NobleBindings.prototype.discoverDescriptors = function (deviceUuid, serviceUuid, characteristicUuid) {
    ...
    this._getCachedCharacteristicAsync(deviceUuid, serviceUuid, characteristicUuid)
        ...
        return rt_utils_1.default
            ...
            .then((result) => {
            checkCommunicationResult(deviceUuid, result);
            let descriptors = rt_utils_1.default.toArray(result.descriptors).map((d) => uuidToString(d.uuid));
...
NobleBindings.prototype._onAdvertisementWatcherReceived = function (sender, e) {
    ...
    debug('watcher received: %s %s %s', getEnumName(BluetoothLEAdvertisementType, e.advertisementType), address, e.advertisement.localName);
    ...
    if (isScanResponse) {
        ...
        serviceUuids = rt_utils_1.default.toArray(e.advertisement.serviceUuids).map((serviceUuid) => {
            if (debug.enabled) {
                debug('	service UUID: %s', getEnumName(GattServiceUuids, serviceUuid) || uuidToString(serviceUuid));
            }
    ...
    let dataSections = rt_utils_1.default.toArray(e.advertisement.dataSections);
    ...
    dataSections.forEach((dataSection) => {
        ...
        switch (dataSection.dataType) {
            case BluetoothLEAdvertisementDataTypes.completeService16BitUuids:
                ...
                var buf = rt_utils_1.default.toBuffer(dataSection.data);
                id[0] = buf[1];
                id[1] = buf[0];
                serviceDataEntry.uuid = id.toString('hex');
                break;
            case BluetoothLEAdvertisementDataTypes.completeService128BitUuids:
                serviceDataEntry.uuid = rt_utils_1.default.toBuffer(dataSection.data).toString('hex');
                break;
            case BluetoothLEAdvertisementDataTypes.serviceData16BitUuids:
                serviceDataEntry.data = rt_utils_1.default.toBuffer(dataSection.data).slice(2);
                break;
            default:
                break;
        }
    ...
    let txPowerDataSection = dataSections.find((ds) => ds.dataType === BluetoothLEAdvertisementDataTypes.txPowerLevel);
    ...
    if (manufacturerSections.size > 0) {
        ...
        deviceRecord.manufacturerData = rt_utils_1.default.toBuffer(manufacturerData.data);
...
NobleBindings.prototype._onConnectionStatusChanged = function (sender, e) {
    ...
    if (deviceRecord) {
        ...
        debug('connection status changed: ' + deviceUuid + ' ' + getEnumName(BluetoothConnectionStatus, connectionStatus));
...
NobleBindings.prototype._getCachedServiceAsync = function (deviceUuid, serviceUuid) {
    ...
    return rt_utils_1.default
        ...
        .then((result) => {
        checkCommunicationResult(deviceUuid, result);
        service = rt_utils_1.default
            .trackDisposables(deviceUuid, rt_utils_1.default.toArray(result.services))
...
NobleBindings.prototype._getCachedCharacteristicAsync = function (deviceUuid, serviceUuid, characteristicUuid) {
    ...
    return this._getCachedServiceAsync(deviceUuid, serviceUuid).then((service) => {
        return rt_utils_1.default
            ...
            .then((result) => {
            checkCommunicationResult(deviceUuid, result);
            characteristic = rt_utils_1.default.toArray(result.characteristics).find((c) => uuidToString(c.uuid) === characteristicUuid);
            if (!characteristic) {
...
NobleBindings.prototype._getCachedDescriptorAsync = function (deviceUuid, serviceUuid, characteristicUuid, descriptorUuid) {
    ...
    return this._getCachedCharacteristicAsync(deviceUuid, serviceUuid, characteristicUuid).then((characteristic) => {
        return rt_utils_1.default
            ...
            .then((result) => {
            checkCommunicationResult(deviceUuid, result);
            descriptor = rt_utils_1.default.toArray(result.descriptors).find((d) => uuidToString(d.uuid) === descriptorUuid);
            if (!descriptor) {
