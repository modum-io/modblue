...
export class MacPeripheral extends Peripheral {
    ...
    public constructor(adapter: MacAdapter, uuid: string, name: string, addressType: AddressType, address: string, manufacturerData: Buffer, rssi: number) {
        super(adapter, uuid, name, addressType, address, manufacturerData, rssi);
    }

    public async connect(options?: ConnectOptions): Promise<GattRemote> {
