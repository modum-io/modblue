...
rt.using(importPrefix + 'Windows.Foundation');
rt.using(importPrefix + 'Windows.Storage.Streams');
rt.using(importPrefix + 'Windows.Devices.Enumeration');
rt.using(importPrefix + 'Windows.Devices.Bluetooth.GenericAttributeProfile');
rt.using(importPrefix + 'Windows.Devices.Bluetooth');
rt.using(importPrefix + 'Windows.Devices.Bluetooth.Advertisement');
rt.using(importPrefix + 'Windows.Devices.Radios');
...
NobleBindings.prototype.discoverIncludedServices = function (deviceUuid, serviceUuid, filterServiceUuids) {
	...
	debug(
		'discoverIncludedServices(%s, %s, %s)',
		deviceUuid,
		serviceUuid,
...
NobleBindings.prototype._onAdvertisementWatcherReceived = function (sender, e) {
	...
	dataSections.forEach((dataSection) => {
		debug(
			'	data section: %s',
			getEnumName(BluetoothLEAdvertisementDataTypes, dataSection.dataType) || dataSection.dataType
		);
...
NobleBindings.prototype._getCachedDescriptorAsync = function (
	deviceUuid,
	serviceUuid,
	characteristicUuid,
	descriptorUuid
